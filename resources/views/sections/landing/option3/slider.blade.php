<section id="slider" 
		 class="slider-element slider-parallax full-screen dark" 
		 style="background: url(https://makstudio.s3.us-east-2.amazonaws.com/backgroundImage2.jpg) center center no-repeat; background-size: cover">

	<div class="slider-parallax-inner">

		<div class="container-fluid vertical-middle clearfix">

			<div class="container clearfix">

				<div class="emphasis-title vertical-middle center">

					<img src="/images/makTextWhite.png" width="200px">

					<div class="heading-block title-center nobottomborder topmargin">
						<h4 class="whiteText">Custom made architectural walls, desks, and more</h4>
					</div>

				</div>

			</div>

		</div>
	</div>

</section>


